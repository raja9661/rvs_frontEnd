...
import { 
  ...
  ShieldCheck, Key, Lock, Loader2, AlertCircle, 
  Check, RefreshCw, Eye, EyeOff, ChevronDown
} from 'lucide-react';
...
const CreateUser = () => {
  ...
  // Fetch creator ID and client codes on mount
  useEffect(() => {
  ...
  // Fetch client codes from backend
  const fetchClientCodes = async () => {
    try {
      setState(prev => ({ ...prev, isFetchingClientCodes: true }));
      const response = await axios.get(`${import.meta.env.VITE_Backend_Base_URL}/mapping/getcode`);
      setState(prev => ({ 
        ...prev, 
        clientCodes: response.data.map(item => item.clientCode),
        isFetchingClientCodes: false 
      }));
    } catch (error) {
      console.error('Error fetching client codes:', error);
      toast.error('Failed to load client codes');
      setState(prev => ({ ...prev, isFetchingClientCodes: false }));
    }
  };

  // Filter client codes based on input
  const filteredClientCodes = state.clientCodeQuery === ''
    ? state.clientCodes
    : state.clientCodes.filter(code =>
        code.toLowerCase().includes(state.clientCodeQuery.toLowerCase())
      );
                  ...
                  >
                    <div className="relative">
                      <Combobox.Input
                        name="clientCode"
                        placeholder={
                          state.isFetchingClientCodes 
                            ? "Loading client codes..." 
                            : "Select or type a client code"
                        }
                        className={`${inputClasses} pl-10 ${
                          state.fieldErrors.clientCode ? 'border-red-500 focus:border-red-500 focus:ring-red-500/20' : ''
                        }`}
                        onChange={(e) => {
                          setState(prev => ({ ...prev, clientCodeQuery: e.target.value }));
                          setFormData(prev => ({ ...prev, clientCode: e.target.value }));
                        }}
                        displayValue={(code) => code}
                        disabled={state.isFetchingClientCodes}
                      />
                      {state.isFetchingClientCodes ? (
                        <Loader2 className="absolute right-3 top-[35px] w-5 h-5 animate-spin text-gray-400" />
                      ) : (
                      ...
                      <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white dark:bg-gray-700 py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                        {filteredClientCodes.length === 0 ? (
                          <div className="relative cursor-default select-none px-4 py-2 text-gray-700 dark:text-gray-300">
                            {state.isFetchingClientCodes ? 'Loading...' : 'No matching client codes found'}
                          </div>
                        ) : (
                          filteredClientCodes.map((code) => (
                            <Combobox.Option
                              key={code}
                              value={code}
                              className={({ active }) =>
